...
;(function ($, window, document, undefined) {
  ...
  var Paging = function (elem, options) {
    ...
    this.$oPrev = this.$oPaging.find('.prev1');
    this.$oNext = this.$oPaging.find('.next1');
    ...
    this.click1 = this.opts.click1; //拼装点击事件
    this.xl = this.opts.xl; //按销量排序
    this.price = this.opts.price; //按价格排序
    this.newpro = this.opts.newpro; //按新品排序
  ...
  Paging.prototype = {
    ...
    detectionPage: function (currentPage) {
      ...
      if (currentPage > 1) {
        ...
        this.$oPrev.attr('onclick',this.click1+'(this,'+this.xl+','+this.price+','+this.newpro+');');
      } else {
        this.$oPrev.addClass('disable');
        this.$oPrev.removeAttr('onclick');
      }

      if (currentPage < this.allPages) {
        this.$oNext.removeClass('disable');
        this.$oNext.attr('onclick',this.click1+'(this,'+this.xl+','+this.price+','+this.newpro+');');
      } else {
        this.$oNext.addClass('disable');
        this.$oNext.removeAttr('onclick');
      }
    },
    ...
    initalPaging: function () {
      for (var i = 1; i <= this.displayPage; i ++) {
        var $create_li = $('<li onclick="'+this.click1+'(this,'+this.xl+','+this.price+','+this.newpro+');"></li>');
